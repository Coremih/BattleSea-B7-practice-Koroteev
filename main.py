...
class BoardOutException(BoardException):
    def __str__(self):
        return "Капитан, наши пушки туда не достанут!"
...
class User(Player):
    def ask(self):
        while True:
            coords = input("Куда будем стрелять? ").split()

            if len(coords) != 2:
                print("Укажите две координаты для выстрела, капитан!")
                continue

            x, y = coords

            if not x.isdigit() or not y.isdigit():
                print("Мы не поняли ваших координат, используйте числа, капитан!")
                continue
    ...
    def loop(self):
        ...
        while True:
            ...
            if self.ai.board.defeat():
                print("-"*20)
                print("Это наша победа, капитан! Сегодня пьём до рассвета!")
                break

            if self.user.board.defeat():
                print("-"*20)
                print("Мы вас подвели, капитан...")
                break
            num +=1
